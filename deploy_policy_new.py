...
roto_2 contract (link-based / shadow_touchlab_col, rl_only_pt):
  - 13 policy actions (control joints); FFJ2/MFJ2/RFJ2 are COMBINED-CURL PROXIES that
    are remapped into J2 + gated J1 targets, replicating roto_env._handle_coupled_joints.
    We therefore publish 16 joints (13 control + FFJ1/MFJ1/RFJ1 mimics).
  - 304-d observation = 4 * (prop 52 + tactile 24)
  - prop per step = [pos_norm(13), vel_norm(13), cmd_error_raw(13), last_action(13)]
  - pos_norm / scale use SIM (USD) limits; FFJ2/MFJ2/RFJ2 upper = 1.745, not 1.5708
  - last_action MUST be the raw policy output (matches sim self.actions), not scaled radians
  - cmd_error uses sim joint_pos_cmd: raw scale for non-coupled, post-coupling J2 for curls

Loads encoder+policy from best_agent.pt and commands joint targets on
/rh_trajectory_controller/command (radians).

See SHADOWLITE_DEPLOY.md for ROS topics and tactile channel map (tactile layout unchanged).
"""
...
# Trained checkpoint (roto_2 link-based, 13-DOF / 304-d):
# scripts/logs/shadowlite_baoding/rl_only_pt/2026-07-10_17-43-40/checkpoints/best_agent.pt
CHECKPOINT = "/home/user/experiments/best_agent_legacy_padtac_new.pt"
...
def main():
    ...
    rospy.init_node("deploy_policy_new")
    ...
    while not biotac_ready and not rospy.is_shutdown():
        rospy.sleep(0.1)
    rospy.loginfo("BioTac states received.")

    t = threading.Thread(target=serial_reader)
    t.daemon = True
    t.start()

    rospy.loginfo("Calibrating FSR + BioTac (hand flat, empty)...")
    rospy.sleep(0.5)
    calibrate_fsr(2.0)
    ...
    if REPLAY_FILE:
        ...
        print('ROllout')
        rec_q = data["q"].astype(np.float32)  # (T, 16) achieved sim joint positions
        assert list(data["joints"]) == PUBLISH_JOINTS, (list(data["joints"]), PUBLISH_JOINTS)
        log = {"t": [], "cmd": [], "q": [], "qd": [], "fsr": [], "biotac_pdc": [],}

        first = np.clip(rec_q[0], PUB_LOWER, PUB_UPPER)
        m = JointTrajectory()
        m.joint_names = PUBLISH_JOINTS
        p = JointTrajectoryPoint()
        p.positions = first.tolist()
        ...
        for q_sim in rec_q:
            ...
            # --- REPLAY-ONLY 16-DOF LOGGING (comment out these 2 lines + use 13-D below) ---
            log["q"].append(current_joint_pos16.copy())   # 16: PUBLISH_JOINTS order
            log["qd"].append(current_joint_vel16.copy())
            # Original 13-D policy-joint logging (uncomment if disabling 16-D above):
            ...
            rate.sleep()
        # --- REPLAY-ONLY 16-DOF LOGGING: also write joints= name list ---
        np.savez(
            "hw_q_replay.npz",
            **{k: np.array(v) for k, v in log.items()},
            joints=PUBLISH_JOINTS,
        )
        # Without 16-D logging, use instead:
        # np.savez("hw_q_replay.npz", **{k: np.array(v) for k, v in log.items()})
        # --- end REPLAY-ONLY 16-DOF LOGGING ---
        rospy.loginfo("saved hw_q_replay.npz: %d steps (q/qd are 16-D PUBLISH_JOINTS)", len(log["t"]))
        return
    ...
    try:
        while not rospy.is_shutdown():
            ...
            # 13-d action -> 16-d hardware command (finger-curl coupling + gov).
            ...
            # Bookkeeping for next obs. last_action = raw policy output (matches sim
            # self.actions). last_command must equal sim joint_pos_cmd for the 13
            # control joints: raw scale for non-coupled, POST-coupling j2_cmd for
            # FFJ2/MFJ2/RFJ2 (used by the cmd_error proprio term).
            last_action[:] = action
        ...
        if rec["t"]:
            np.savez("hw_policy_log_padtacbt_100.npz", **{k: np.array(v) for k, v in rec.items()})
            rospy.loginfo("saved hw_policy_log_padtac_new.npz: %d steps", len(rec["t"]))
