#!/usr/bin/env python
"""Gate C / notac deploy: SIM / ZERO / REAL tactile.  [DIAGNOSTIC COPY]

This is deploy_policy_simtactile.py (the mass-diff / notac variant, with the
scaled_error / USE_MLP_POS_ERR machinery) with a live pos_err diagnostic bolted
on so the "policy repeats the same motion and never recovers" symptom can be
inspected directly on hardware.

NO control-path values were changed: scaled_error, SPEED_FRAC, TACTILE_MODE,
USE_MLP_POS_ERR, USE_PUBLISHED_CMD_FOR_POS_ERR, POS_ERR_SPEED_FRAC, CHECKPOINT
all keep their original defaults below. What was added (search "DIAG"):
  - every DIAG_PRINT_EVERY steps, print the pos_err decomposition:
      raw_err      = last_command[t-1] - q[t]          (pre-scale, what build_prop uses)
      tracking_err = published_cmd13[t] - q[t]          (sim-style error, no slew gap)
      slew_gap     = raw_err - tracking_err             (== last_command[t-1] - published[t])
      obs_err      = scaled_error * raw_err             (the literal value fed to the policy)
    A persistently positive obs_err on the curl joints (FFJ2/MFJ2/RFJ2, CURL_J2_IDX)
    is the "policy thinks it never reached the target" signature.
  - those four arrays are additionally saved into LOG_NPZ per-step for offline analysis
    (see scratchpad/diagnose_pos_err.py, which computes the same decomposition offline).

Edit PROTOCOL flags at the top of this file, then:
 python deploy_policy_simtactile_diag.py
...
LOG_NPZ = "hw_policy_log_proponlyrandomweights.npz"
...
def build_prop():
   ...
   return np.concatenate([pos_norm, vel_norm, scaled_error*error, last_action]).astype(np.float32)
...
def diag_pos_err(pub_target, q_now):
   """Decompose this step's pos_err exactly like scratchpad/diagnose_pos_err.py.
...
def diag_print(step_i, raw_err, tracking_err, slew_gap, obs_err):
   ...
   print(
       ...
       "obs_err(x%s) curl_bias=%+.3f rad%s"
       % (step_i, l2_raw, l2_track, l2_slew, scaled_error, curl_bias, flag)
   )
...
def main():
   ...
   rospy.init_node("deploy_policy_simtactile_diag")
   ...
   input(
       ...
       "(TACTILE_MODE=%s, plant SPEED_FRAC=%s, MLP=%s, POS_ERR_SPEED_FRAC=%s)..."
       % (TACTILE_MODE, SPEED_FRAC, USE_MLP_POS_ERR, POS_ERR_SPEED_FRAC)
   )
   ...
   if USE_MLP_POS_ERR:
       ...
       rospy.loginfo(
           ...
           "(raw / tracking(sim-like) / slew_gap / obs=x%s). curl_bias warn > %.3f rad",
           DIAG_PRINT_EVERY, scaled_error, DIAG_CURL_BIAS_WARN,
       )
